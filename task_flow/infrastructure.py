...
Technical notes: Terraform HCL configuration targeting the AWS provider. Declares aws_dynamodb_table (PAY_PER_REQUEST, GSI on status), two aws_lambda_function resources sharing one IAM role with least-privilege inline policies (DynamoDB PutItem/Scan + SES SendEmail), aws_apigatewayv2_api HTTP API with two routes and Lambda integrations, aws_scheduler_schedule for the notify Lambda (daily 08:00 UTC), aws_ses_email_identity for sender verification, input variables with sensitive flags, and an S3+DynamoDB remote backend.
"""
...
# ------------------------------------------------------------
# variables.tf (config)
# Declares all tuneable inputs for the deployment: AWS region, DynamoDB table name, the sender email address, and the notification recipient. Sensitive values must be supplied at plan/apply time and must never be committed to source control.
# ------------------------------------------------------------
# variables.tf
# ---------------------------------------------------------------
...
# ------------------------------------------------------------
# backend.tf (config)
# Tells Terraform to store its state file safely in an S3 bucket with encryption, and use a DynamoDB table to prevent two people applying changes at the same time.
# ------------------------------------------------------------
# backend.tf
# ---------------------------------------------------------------
...
# ------------------------------------------------------------
# aws_dynamodb_table.tasks (config)
# Creates the DynamoDB table that stores all tasks. Uses on-demand billing so you only pay per request, enables automatic backups so you can restore to any point in the last 35 days, and encrypts all data at rest.
# ------------------------------------------------------------
# dynamodb.tf
# ---------------------------------------------------------------
...
# ------------------------------------------------------------
# aws_lambda_function.api (config)
# Packages and deploys the API Lambda that handles HTTP requests from API Gateway. Gives it just enough permissions to read/write the tasks table and nothing more, plus the ability to send emails via SES.
# ------------------------------------------------------------
# lambda_api.tf
# ---------------------------------------------------------------
...
# ------------------------------------------------------------
# aws_lambda_function.notify (config)
# Deploys the scheduled Lambda that checks for overdue tasks and sends the daily digest email. An EventBridge Scheduler rule fires it every morning at 08:00 UTC with a 5-minute flexible window to smooth AWS capacity.
# ------------------------------------------------------------
# lambda_notify.tf
# ---------------------------------------------------------------
...
# ------------------------------------------------------------
# aws_apigatewayv2_api.taskflow (config)
# Sets up the public HTTP API that routes GET and POST requests on the /tasks path to the API Lambda, and outputs the URL you call from a browser or mobile app.
# ------------------------------------------------------------
# apigw.tf
# ---------------------------------------------------------------
...
# ------------------------------------------------------------
# aws_ses_email_identity.sender (config)
